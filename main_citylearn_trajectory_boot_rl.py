...
from trajectory.models.gpt.gpt_trainer_boot import GPTTrainerBoot
...
from eval_traj import aug_trajectory,eval_trajectory,test_trajectory
...
class Experiment:
    def __init__(self, variant,dataset_path,config_path = "configs/medium/city_learn_traj_v1_seq20_rf_CombinedReward_norm_wrapper.yaml"):
        ...
        final_tokens = warmup_tokens * num_epochs * 20
    ...
    def pretrain(self,schema_eval = "citylearn_challenge_2022_phase_2"):
        ...
        #print("DEVICEE")
        #print(self.device)
        #env_dataset_2,df_evaluate_2 = eval_trajectory(model = self.model,discretizer = self.discretizer,config = self.eval_config,schema= "citylearn_challenge_2022_phase_2",device = self.device)
        env_test, df_test, df_ts_test = test_trajectory(model = self.model,discretizer = self.discretizer,config = self.eval_config,schema= "citylearn_challenge_2022_phase_3",device = self.device)
        df_test.to_csv(self.logger.log_path+f"/final_test_schema_3.csv")
        df_ts_test.to_csv(self.logger.log_path+f"/final_ts_schema_3.csv")

        data_path_test = self.logger.log_path + "/data_interaction_3.pkl"
        Dataset.from_dict({k: [s[k] for s in env_test] for k in env_test[0].keys()}).save_to_disk(data_path_test)

        


        env_dataset_1,df_evaluate_1,df_ts1 = eval_trajectory(model = self.model,discretizer = self.discretizer,config = self.eval_config,schema= "citylearn_challenge_2022_phase_1",device = self.device)

        df_ts1.to_csv(self.logger.log_path+f"/final_ts_schema_1.csv")
        df_evaluate_1.to_csv(self.logger.log_path+f"/final_eval_schema_1.csv")

        data_path_1 = self.logger.log_path + "/data_interaction_1.pkl"
        Dataset.from_dict({k: [s[k] for s in env_dataset_1] for k in env_dataset_1[0].keys()}).save_to_disk(data_path_1)
        ...
        #df_evaluate_2.to_csv(self.logger.log_path+f"/final_eval_schema_2.csv")
    ...
    def online_tuning(self,  online_schema, schema_eval):
        ...
        while self.online_iter < self.variant["max_online_iters"]:
            ...
            aug_dataset,aug_df_evaluate = aug_trajectory(model = self.model,discretizer = self.discretizer,config = self.eval_config,schema= online_schema,device = self.device)
            ...
            env_dataset,df_evaluate =  eval_trajectory(model = self.model,discretizer = self.discretizer,config = self.eval_config,schema= schema_eval,device = self.device)
...
def run_experiment(seed):
    ...
    # pretraining options
    parser.add_argument("--max_pretrain_epochs", type=int, default=50)
